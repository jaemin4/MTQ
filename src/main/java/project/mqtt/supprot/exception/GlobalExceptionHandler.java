package project.mqtt.supprot.exception;
...
import project.mqtt.supprot.response.APIResponse;
...
public class GlobalExceptionHandler {
    ...
    public APIResponse<Object> handleMissingServletRequestParameterException(MissingServletRequestParameterException e) {
        ...
        log.warn("[MissingServletRequestParameterException] 필수 요청 파라미터 '{}'가 누락되었습니다.", paramName);
        log.debug("[MissingServletRequestParameterException stack]", e);

        return APIResponse.fail(HttpStatus.BAD_REQUEST.value(),
                String.format("필수 요청 파라미터 '%s'가 누락되었습니다.", paramName));
    }
